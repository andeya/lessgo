	...
	walk:
		for {
			...
			if i < len(n.path) {
				...
				n.indices = utils.Bytes2String([]byte{n.path[i]})
				n.path = path[:i]
				n.handle = nil
				n.wildChild = false
			}
			...
			if i < len(path) {
				...
				if c != ':' && c != '*' {
					// []byte for proper unicode char conversion, see #65
					n.indices += utils.Bytes2String([]byte{c})
					child := &node{
						maxParams: numParams,
					}
...
// given path.
func (n *node) getValue(path string, pnames, pvalues []string) (HandlerFunc, []string, []string, bool) {
	var (
		handle HandlerFunc
		tsr    bool
	)
	...
	if pvalues == nil {
		// lazy allocation
		pvalues = make([]string, 0, n.maxParams)
	}
		...
		if len(path) > len(n.path) {
			if path[:len(n.path)] == n.path {
				...
				if !n.wildChild {
					...
					return handle, pnames, pvalues, tsr
				}
